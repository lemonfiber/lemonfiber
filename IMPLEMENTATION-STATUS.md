...
| Credential validation against live services | `A3-R1..R4`, `A3-R10` | ◐ | The validation primitive is built: [`servarr.rs`](crates/lemonfiber-core/src/servarr.rs)'s `identity()` reads a service's `system/status` over the HTTP transport ([`ports/http.rs`](crates/lemonfiber-core/src/ports/http.rs) + reqwest/rustls [`adapters/http.rs`](crates/lemonfiber-core/src/adapters/http.rs)), distinguishing rejected (`401/403`), unreachable (no answer), and unusable (the service's own words, verbatim — `A3-R4`). The doctor check that runs it needs the API keys, which come from config.xml extraction (next). |
