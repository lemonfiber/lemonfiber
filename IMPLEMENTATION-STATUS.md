...
| Empirical hardlink test (create / `stat` / inode + link-count) | `C5-R1`, `A2-R8` | ✅ | [`doctor/storage.rs`](crates/lemonfiber-core/src/doctor/storage.rs); real disk, symlink-resolved (`C5-R13`) |
| Storage-mode detection (fs type, network mount, exFAT, WSL2) | `C5-R2`, `C5-R14` | ◐ | mode derived from the probe everywhere; fstype naming reads the mount table on Linux, environment-named elsewhere; consequence-framed (`C5-R3`) |
