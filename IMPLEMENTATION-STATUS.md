...
| Credential validation against live services | `A3-R1..R4`, `A3-R10` | ◐ | The validation primitive is built: [`servarr.rs`](crates/lemonfiber-core/src/servarr.rs)'s `identity()` reads a service's `system/status` over the HTTP transport ([`ports/http.rs`](crates/lemonfiber-core/src/ports/http.rs) + reqwest/rustls [`adapters/http.rs`](crates/lemonfiber-core/src/adapters/http.rs)), distinguishing rejected (`401/403`), unreachable (no answer), and unusable (the service's own words, verbatim — `A3-R4`). `servarr::api_key` now reads a service's generated key from its config.xml (`D1-R1`; empty/absent = not-generated-yet, skip and retry). What remains is the doctor check that reads the file through the filesystem port and runs `identity()`. |
...
| Servarr-shape adapter (`identity`, register client/root folder) | `D1-*` | ◐ | [`servarr.rs`](crates/lemonfiber-core/src/servarr.rs): one `Client` for all four Servarr apps, on the HTTP port. `identity` + `register_download_client`/`register_root_folder` post to the versioned API with the key; a transport failure is `Unavailable`, `401/403` is `Unauthorised`, other refusals carry the service's verbatim message. Fake-`Http`-tested. The per-implementation download-client field schema (SABnzbd vs qBittorrent) is filled in when seed wires a concrete client. |
| Seed orchestration (skip-if-absent, preserve operator edits) | `D1-*` | ◐ | [`seed.rs`](crates/lemonfiber-core/src/seed.rs): the pure policy is done — `intent` maps an observed connection to leave-it-alone (present & correct → idempotent, `D1-R2`), preserve-it (operator drift → `D1-R3`), skip-it (prerequisite unavailable → `D1-R5`), or write-it; the `Report` names exactly which connections a re-run still owes (`D1-R6`). The observe-and-execute driver — reading existing connections through the port, writing, verifying, journalling — comes next. |
