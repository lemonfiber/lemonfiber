...
use lexed::{comments, Rule, Violation, RULES};
use source_tree::{shipped, sources, workspace_root};
...
/// A file that ends mid-token is read to its end and no further.
///
/// Every one of these is a file somebody is halfway through writing, and the reader
/// meets them in an editor's save as readily as in a merge. One that ran off the end
/// or went round for ever would take the gate with it, and a guard that cannot finish
/// is a guard that reports nothing — the same silence as a guard that found nothing.
#[test]
fn a_file_that_ends_mid_token_is_read_no_further() {
    let unclosed = [
        ("a string", "let s = \"// not a comment", 0),
        ("a raw string", "let s = r#\"// not a comment", 0),
        ("a character", "let c = '\\", 0),
        ("a character over a line break", "let c = '\\\n;\n", 0),
        ("a lifetime", "fn held<'a", 0),
        ("a block comment", "/* nobody closed this", 1),
    ];
    assert_eq!(
        unclosed.len(),
        6,
        "nothing unclosed was put in front of the reader"
    );

    for (what, half, expected) in unclosed {
        let found = comments(half).len();
        assert_eq!(
            found, expected,
            "{what}: the reader made {found} comments of it"
        );
    }
}
