...
/// Every declaration this workspace ships, with the file it is in.
fn declared() -> BTreeMap<String, Vec<String>> {
    shipped()
        .into_iter()
        .map(|(path, text)| (path, declarations(&text)))
        .collect()
}
...
fn nothing_declares_a_second_account_for_what_a_plugin_brings() {
    ...
    let counted: usize = declared.values().map(Vec::len).sum();
    assert!(
        counted > 100,
...
fn the_rule_refuses_a_second_account_of_each_of_the_three() {
    for account in ACCOUNTS {
        let planted = planted(&format!("Plugin{}s", first(account.spelled)));
        let found = second_accounts(&planted);
        assert_eq!(
            found.len(),
            1,
            "a planted second account of {} was read as {found:?}",
            account.named
        );
        let Some(second) = found.first() else {
            unreachable!("a list of one has a first")
        };
        assert_eq!(second.of, account.named);
        assert_eq!(second.kept, account.kept);
        assert!(
            second.named.to_lowercase().contains(first(account.spelled)),
            "the refusal names the declaration rather than only the account: {second:?}"
        );
    }
}
