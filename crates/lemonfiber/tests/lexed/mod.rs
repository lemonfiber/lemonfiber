...
//!
//! So the reader is a lexer. It carries the state a Rust file needs — a string, a
//! raw string with any number of hashes, a character literal, a lifetime that is not
//! one, and a block comment that may hold another — and it reports comments rather
//! than code, which is the half the rules below are about.
//!
...
pub(crate) fn comments(text: &str) -> Vec<Comment> {
    ...
    let mut at = 0;
    while at < source.len() {
        if let Some((until, block, doc)) = opening(&source, at) {
            found.push(Comment {
                line: numbered(&source, at),
                doc,
                block,
                trailing: preceded(&source, at),
                said: source.get(at..until).unwrap_or_default().iter().collect(),
            });
            at = until;
            continue;
        }
...
/// Where the comment opening here ends, and what kind it is.
fn opening(source: &[char], at: usize) -> Option<(usize, bool, bool)> {
    if reads(source, at, "//") {
        let doc =
            (reads(source, at, "///") && !reads(source, at, "////")) || reads(source, at, "//!");
        let mut cursor = at + 2;
        while source.get(cursor).is_some_and(|letter| *letter != '\n') {
            cursor += 1;
        }
        return Some((cursor, false, doc));
    }
    if !reads(source, at, "/*") {
        return None;
    }
    // `/**/` is an empty block comment rather than the shortest doc one, which is
    // the same call rustc makes and the reason the closing marker is read first.
    let doc = (reads(source, at, "/**") && !reads(source, at, "/**/")) || reads(source, at, "/*!");
    let mut cursor = at + 2;
    let mut depth = 1_usize;
    while cursor < source.len() {
        if reads(source, cursor, "*/") {
            depth -= 1;
            cursor += 2;
            if depth == 0 {
                break;
            }
        } else if reads(source, cursor, "/*") {
            depth += 1;
            cursor += 2;
        } else {
            cursor += 1;
    ...
    Some((cursor, true, doc))
}

/// Past whatever token stands here, which is one character where it is code.
///
/// Strings, raw strings and character literals are stepped over whole. They are the
/// reason this is a lexer: each of them can hold two slashes, and one of them can
/// hold a quote.
fn beyond(source: &[char], at: usize) -> usize {
    if let Some(after) = raw_string(source, at) {
        return after;
    }
    match source.get(at) {
        Some('"') => text(source, at),
        Some('\'') => quoted(source, at),
        _ => at + 1,
    }
}

/// Past a `"…"` string, whose escapes may hold a quote.
fn text(source: &[char], at: usize) -> usize {
    let mut cursor = at + 1;
    while let Some(letter) = source.get(cursor) {
        match letter {
            '\\' => cursor += 2,
            '"' => return cursor + 1,
            _ => cursor += 1,
...
/// Past a raw string opening here, which closes only at a quote and its hashes.
fn raw_string(source: &[char], at: usize) -> Option<usize> {
    if at > 0 && source.get(at - 1).copied().is_some_and(named) {
        return None;
    }
    let opened = ["br", "cr", "r"]
        .into_iter()
        .find(|prefix| reads(source, at, prefix))
        .map(|prefix| at + prefix.len())?;
    let mut hashes = 0;
    while source.get(opened + hashes) == Some(&'#') {
        hashes += 1;
    }
    if source.get(opened + hashes) != Some(&'"') {
        return None;
    }

    let mut cursor = opened + hashes + 1;
    while cursor < source.len() {
        if source.get(cursor) == Some(&'"')
            && (1..=hashes).all(|step| source.get(cursor + step) == Some(&'#'))
        {
...
/// Past a character literal, or past the lifetime that is written the same way.
///
/// `'a'` and `'a` differ only in what follows, and reading a lifetime as an unclosed
/// character literal would swallow the rest of the file — every comment after it
/// included, which is a gate that reports nothing and looks as though it checked.
fn quoted(source: &[char], at: usize) -> usize {
    if source.get(at + 1) == Some(&'\\') {
        let mut cursor = at + 2;
        while let Some(letter) = source.get(cursor) {
            match letter {
                '\n' => return cursor,
    ...
    if source.get(at + 2) == Some(&'\'') {
        return at + 3;
    }
    let mut cursor = at + 1;
    while source.get(cursor).copied().is_some_and(named) {
        cursor += 1;
    }
    cursor.max(at + 1)
}

/// Whether the text at this offset opens with these characters.
fn reads(source: &[char], at: usize, word: &str) -> bool {
    word.chars()
        .enumerate()
        .all(|(step, letter)| source.get(at + step) == Some(&letter))
}
...
fn numbered(source: &[char], at: usize) -> usize {
    source
        .iter()
        .take(at.min(source.len()))
        .filter(|letter| **letter == '\n')
        .count()
        + 1
}

/// Whether anything but whitespace stands before this offset on its own line.
fn preceded(source: &[char], at: usize) -> bool {
    source
        .iter()
        .take(at)
        .rev()
        .take_while(|letter| **letter != '\n')
        .any(|letter| !letter.is_whitespace())
}
