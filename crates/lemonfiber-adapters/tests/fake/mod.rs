...
//!
//! A module of its own rather than one inside the file that first needed it, because
//! two test binaries now drive the adapter and each would otherwise carry a copy —
//! and two engines that were meant to answer the same way are two engines that will
//! eventually not.

// Each test binary that declares this module compiles all of it and uses the part it
// needs: one drives listings and streams, the other execs, and neither reaches every
// reply shape.
#![allow(dead_code)]
