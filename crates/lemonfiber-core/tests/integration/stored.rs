...
fn ctx(eraser: Arc<Erasing>) -> Ctx {
    lemonfiber_testing::a_live_context()
        .build()
        // A vault this run never reaches. `Archiving` is where a context holds the
        // layout, and the layout is the whole of what these two commands read from it —
        // so what is beside it answers nothing and is asked nothing.
...
async fn a_run_that_does_not_know_where_its_files_are_is_refused() {
    let bare = lemonfiber_testing::a_live_context().build();
