...
use super::{adopting, mounting, over, scratch, somebody_elses, theirs};
...
async fn adopting_unconfirmed_says_what_it_would_do_and_writes_nothing() {
    ...
    let found = adopting(&theirs("4.0.0", Some(env.to_path_buf())), false).await;
    assert_eq!(
...
async fn confirming_records_the_project_lemonfiber_now_manages() {
    ...
    let found = adopting(&theirs("4.0.15", Some(env.to_path_buf())), true).await;
    assert_eq!(
        found
            .as_ref()
...
async fn adopting_that_cannot_write_its_answer_reports_the_failure() {
    let blocked = lemonfiber_fixtures::scratch::Scratch::named("blocked");
