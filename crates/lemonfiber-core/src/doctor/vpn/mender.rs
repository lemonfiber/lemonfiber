...
/// Moving the client, and every way that can end.
///
/// Outside the trait so the coverage report can watch it decide: a branch inside an
/// `#[async_trait]` body has no region of its own, so a repair whose four outcomes are
/// all written here reads as measured whether or not any of them has ever run.
async fn mended(mender: &PortMender) -> Attempt {
    let Some(client) = &mender.client else {
    ...
    let forwarding = Forwarding {
        granted: grant_at(mender.engine.as_ref(), &mender.project, &mender.gateway).await,
