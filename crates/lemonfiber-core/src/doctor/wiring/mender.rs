...
///
/// Outside the trait for the reason the mend below is: a branch inside an
/// `#[async_trait]` body gets no coverage region of its own, so the refusals here
/// read as measured whether or not any of them has ever run.
...
/// Putting the category back, and every way that can end.
async fn mended(mender: &WiringMender, repair: &Repair) -> Attempt {
    let Some((managed, wired)) = mender.named(repair) else {
