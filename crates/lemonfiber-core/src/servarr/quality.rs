...
impl MusicQuality for Servarr {
    async fn apply_music_format(&self, format: crate::audio::Format) -> Result<(), Failure> {
        apply_music_format(self, format).await
    }
}
...
async fn apply_music_format(
    ...
    if prefer {
        servarr.ensure_hi_res_format().await?;
    }
    let response = servarr
        .probe(&servarr.request(Method::Get, "/qualityprofile", None))
        .await?;
    let profiles: Vec<serde_json::Value> = servarr
        .endpoint
    ...
    for profile in profiles {
        ...
        let body = crate::lidarr::set_hi_res_preference(&rewritten, prefer).unwrap_or(rewritten);
        let Some(id) = profile.get("id").and_then(serde_json::Value::as_i64) else {
            continue;
        };
        let updated = servarr
            .probe(&servarr.request(Method::Put, &format!("/qualityprofile/{id}"), Some(body)))
            .await?;
        servarr.endpoint.expect_success(&updated)?;
    }
    Ok(())
}

async fn probe_releases(servarr: &Servarr, id_param: &str) -> Result<ReleaseProbe, Failure> {
    ...
    let response = servarr
        .probe(&servarr.request(Method::Get, "/wanted/missing?page=1&pageSize=1", None))
        .await?;
    let wanted: Wanted = servarr
        .endpoint
    ...
    let response = servarr
        ...
        .await?;
    let releases: Vec<ReleaseResource> = servarr
        .endpoint
