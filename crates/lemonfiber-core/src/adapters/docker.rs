...
fn cpu_fraction(sample: &ContainerStatsResponse) -> f64 {
    let Some(current) = sample.cpu_stats.as_ref() else {
        return 0.0;
    };
    let Some(previous) = sample.precpu_stats.as_ref() else {
        return 0.0;
    };
...
mod tests {
    ...
    use super::{
        cpu_fraction, describe, exit_code, health, lifecycle, sampled, split_timestamp, Daemon,
    };
    use crate::ports::docker::{Engine as _, Failure, Health, Lifecycle};

    /// A container summary as the engine would report one.
    fn summary(service: &str, state: ContainerSummaryStateEnum, status: &str) -> ContainerSummary {
        let mut labels = std::collections::HashMap::new();
        labels.insert(super::PROJECT_LABEL.to_owned(), "lemonfiber".to_owned());
        labels.insert(super::SERVICE_LABEL.to_owned(), service.to_owned());

        ContainerSummary {
            id: Some(format!("id-{service}")),
            labels: Some(labels),
            state: Some(state),
    ...
    #[test]
    fn a_container_is_correlated_back_to_the_service_that_declared_it() {
        let mut reported = summary("sonarr", ContainerSummaryStateEnum::RUNNING, "Up 2 minutes");
        reported.health = Some(ContainerSummaryHealth {
            status: Some(ContainerSummaryHealthStatusEnum::HEALTHY),
            ..Default::default()
        });

        let described = describe(reported);
        assert_eq!(
            (
                described.id,
                described.project,
    ...
    #[test]
    fn a_container_nobody_labelled_is_described_rather_than_dropped() {
        let described = describe(ContainerSummary::default());
        assert_eq!(
            (
                described.project.as_str(),
                described.service.as_str(),
                described.id.as_str(),
                described.lifecycle
            ),
            ("", "", "", Lifecycle::Dead),
            "an unlabelled container is not this stack's, and says so by being empty"
        );
    }

    #[test]
    fn every_state_the_engine_can_report_has_a_meaning_here() {
        for (reported, expected) in [
            (ContainerSummaryStateEnum::CREATED, Lifecycle::Created),
            (ContainerSummaryStateEnum::RUNNING, Lifecycle::Running),
            (ContainerSummaryStateEnum::PAUSED, Lifecycle::Paused),
            (ContainerSummaryStateEnum::RESTARTING, Lifecycle::Restarting),
            (ContainerSummaryStateEnum::EXITED, Lifecycle::Exited),
            (ContainerSummaryStateEnum::REMOVING, Lifecycle::Removing),
            (ContainerSummaryStateEnum::STOPPING, Lifecycle::Removing),
            (ContainerSummaryStateEnum::DEAD, Lifecycle::Dead),
            (ContainerSummaryStateEnum::EMPTY, Lifecycle::Dead),
        ] {
            assert_eq!(lifecycle(Some(reported)), expected, "{reported:?}");
        }
        assert_eq!(lifecycle(None), Lifecycle::Dead, "silence is not running");
    }

    #[test]
    fn every_verdict_a_probe_can_reach_has_a_meaning_here() {
        for (reported, expected) in [
            (ContainerSummaryHealthStatusEnum::STARTING, Health::Starting),
            (ContainerSummaryHealthStatusEnum::HEALTHY, Health::Healthy),
            (
                ContainerSummaryHealthStatusEnum::UNHEALTHY,
                Health::Unhealthy,
            ),
            (ContainerSummaryHealthStatusEnum::NONE, Health::None),
            (ContainerSummaryHealthStatusEnum::EMPTY, Health::None),
        ] {
            assert_eq!(health(Some(reported)), expected, "{reported:?}");
        }
        assert_eq!(health(None), Health::None, "a container without a probe");
    }

    #[test]
    fn a_container_that_stopped_carries_how_it_exited() {
        assert_eq!(exit_code(Some("Exited (137) 2 hours ago")), Some(137));
        assert_eq!(exit_code(Some("Exited (0) About a minute ago")), Some(0));
    }

    #[test]
    fn a_status_that_is_not_an_exit_reports_no_code() {
        for status in [
            ...
            let read = exit_code(Some(status));
            assert_eq!(
                read.is_some() && !status.starts_with("Exited"),
                status.starts_with("Restarting"),
                "{status:?} read as {read:?}"
            );
        }
        assert_eq!(exit_code(None), None);
        assert_eq!(exit_code(Some("Exited but unparenthesised")), None);
        assert_eq!(exit_code(Some("Exited )backwards(")), None);
    }

    #[test]
    fn a_line_the_container_timestamped_keeps_its_instant() {
        assert_eq!(
            split_timestamp("2026-07-25T18:40:55.123456789Z import complete"),
            (
    ...
    #[test]
    fn a_line_without_an_instant_is_passed_through_whole() {
        for line in ["no timestamp here", "single", "2026-07-25 not-a-stamp"] {
            assert_eq!(
                split_timestamp(line),
                (None, line.to_owned()),
                "{line:?} must not lose its first word to a guess"
            );
        }
    }

    /// A pair of CPU samples, the second `used` further on than the first.
    fn cpu(used: u64, elapsed: u64, cores: u32) -> ContainerStatsResponse {
        let at = |total: u64, system: u64| ContainerCpuStats {
        ...
        ContainerStatsResponse {
            cpu_stats: Some(at(used, elapsed)),
            precpu_stats: Some(at(0, 0)),
    ...
    #[test]
    fn cpu_use_is_reported_as_a_fraction_of_one_core() {
        let half_of_one = cpu(500, 1000, 1);
        assert!((cpu_fraction(&half_of_one) - 0.5).abs() < 0.001);

        // A quarter of the machine's time across four cores is one whole core.
        let one_of_four = cpu(250, 1000, 4);
        assert!((cpu_fraction(&one_of_four) - 1.0).abs() < 0.001);
    }

    #[test]
    fn the_first_sample_reports_no_use_rather_than_an_invented_rate() {
        assert!((cpu_fraction(&cpu(500, 0, 1))).abs() < f64::EPSILON);

        let no_previous = ContainerStatsResponse {
            cpu_stats: Some(ContainerCpuStats::default()),
            ..Default::default()
        };
        assert!(cpu_fraction(&no_previous).abs() < f64::EPSILON);
        assert!(cpu_fraction(&ContainerStatsResponse::default()).abs() < f64::EPSILON);
    }

    #[test]
    fn a_sample_carries_resident_memory_alongside_the_rate() {
        let taken = sampled(&cpu(500, 1000, 1));
        assert_eq!(taken.memory_bytes, 2048);
        assert!((taken.cpu - 0.5).abs() < 0.001);

        assert_eq!(
            sampled(&ContainerStatsResponse::default()).memory_bytes,
            0,
            "an engine that did not say is not a container using no memory"
        );
    }

    #[tokio::test]
    async fn asking_the_engine_this_machine_has_never_reports_a_container_that_is_not_there() {
        ...
        // unexercised, or an absent daemon being reported as something an
        // operator would go looking for a container about.
        let outcome = Daemon::local().list("lemonfiber-no-such-project").await;
        assert!(
