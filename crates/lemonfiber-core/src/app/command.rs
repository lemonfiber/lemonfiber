...
pub enum Command {
    ...
    /// Move the stack onto the image versions this build pins.
    ///
    /// Unconfirmed it says what would change and touches nothing; confirmed it takes
    /// those steps, one service at a time, behind a backup. The two are one command
    /// because they are one request read twice — what a browser agrees to is what it
    /// was shown.
    Update(update::Asked),
