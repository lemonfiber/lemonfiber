...
"#;
    ...
    async fn a_household_service_is_put_on_the_proxy_and_the_dashboard_and_taken_off_again() {
        ...
        assert!(
            read(&proxy).contains("komga.{$DOMAIN:home.local} {\n\treverse_proxy komga:25600\n}")
        );
...
id          = "shelf""#,
    ...
    const REPINNED: &str =
        "sha256:5f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945";
    ...
    async fn on(ctx: &Ctx) -> Option<String> {
        report(reading(ctx).await)
            .and_then(|one| one.installed.first().map(|one| one.version.clone()))
    }
...
service = "komga""#,
    ...
    async fn an_update_whose_record_cannot_be_written_puts_the_old_version_back() {
        ...
        assert!(failed
            .as_ref()
            .is_some_and(|one| one.stopped.as_deref().is_some_and(
                |why| why.contains("record of what is installed could not be written")
            )));
        assert!(failed.is_some_and(|one| one.restored.is_some()));
        assert_eq!(on(&ctx).await.as_deref(), Some("1.2.0"));
    }
