...
pub(super) fn command(action: &str, given: Arguments) -> Option<Command> {
    named(action, given).ok()
}

/// Why an action was refused, or nothing where it was not.
pub(super) fn refusal(action: &str, given: Arguments) -> Option<Refused> {
...
pub(super) fn swept(
    argument: &str,
    give: fn(&mut Arguments),
    carries: fn(&Command) -> bool,
) -> Vec<String> {
