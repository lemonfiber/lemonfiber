...
async fn every_read_this_surface_serves_refuses_a_parameter_no_read_takes() {
    ...
    for read in table::OFFERED
        .iter()
        .copied()
        .chain(std::iter::once(table::LOGS))
    {
...
fn a_name_that_reaches_no_read_takes_no_parameter_either() {
    ...
    let refused = table::wanted("/api/secrets", Some("key=LEMONFIBER_USENET"));
...
async fn accounting_for_the_line_takes_nothing_and_refuses_a_parameter() {
    ...
    // declaration goes.
    let refused = table::wanted(table::BANDWIDTH, Some("down=50%"));
...
async fn accounting_for_the_disk_takes_nothing_and_refuses_a_parameter() {
    ...
    let refused = table::wanted(table::SPACE, Some("reclaim=seeding"));
...
async fn the_enumeration_takes_nothing_and_refuses_a_parameter() {
    ...
    let refused = table::wanted(table::OUTBOUND, Some("reach=registry"));
...
async fn a_run_that_cannot_say_where_its_files_are_refuses_to_list_them() {
    ...
    // be somebody else's.
    let seen = asked(world(running(), stack()), table::STORED).await;
...
async fn asking_where_this_copy_stands_about_two_versions_at_once_is_refused() {
    ...
    let refused = table::wanted(table::UPDATE, Some("to=0.9.0&to=0.10.0"));
...
fn a_shelf_asked_for_with_no_count_is_read_to_what_one_screen_holds() {
    let named = table::wanted(table::HELD, Some("member=ada"))
        .ok()
        .and_then(|given| table::named(table::HELD, given).ok());
