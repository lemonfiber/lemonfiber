...
//!
//! `tests/` compiles each file as its own crate and this module into every one of
//! them, so anything one file uses is unused in the others — a property of how
//! integration tests are built rather than of what is written here. Both lints are
//! suppressed for that and no other reason.
...
pub(crate) use lemonfiber_core::admission::{credential, Credential};
...
pub(crate) use lemonfiber_fixtures::support::{a_password, Reporting};
