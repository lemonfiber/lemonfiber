...
| `migrate` | `/api/migration`, `migrate-adopt`, `migrate-beside` | dashboard | Reachable in full from all three. The survey is the read — the stacks already standing on this machine, the ports they hold that lemonfiber would want, what it could not take over, what taking it over would cost, and what an unlinkable layout costs — and it starts, stops, moves and writes nothing. Adopting is the act beside it, and the only part of migration that writes: one line saying which Compose project lemonfiber manages. Unconfirmed it says what it would do and writes nothing, so what a browser agrees to is what it was shown. Standing beside is the second act: it writes a Compose file of lemonfiber's own, layered over the stack's, saying where each service listens instead so the two do not want the same ports — and touches nothing of theirs at all. Importing and replacing are described by the survey and not yet offered; each will arrive as a name of its own rather than as an argument to one of these, because standing a second stack up and stopping somebody's are not the same act under a flag. |
