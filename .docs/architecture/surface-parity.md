...
| `self-update` | `/api/update` | dashboard | Reachable in full from all three, and the row where what is *not* offered is the whole design: no surface replaces the binary, because the thing that would go wrong is silent and arrives weeks later — a copy that overwrote itself underneath Homebrew leaves the next `brew upgrade` producing something the operator did not ask for and cannot explain. So all three answer the same question and hand back the same line to copy: which tool owns this copy, whether anything newer has been released, and the exact command for that tool. `--to` is one query parameter and one flag, and it is the only thing any of them takes — it asks about one particular version instead of whatever is newest, which is how going back is asked for, and it carries the sentence saying whether that version reads the configuration already here. It reaches the command from a browser through the same read table the terminal asks through, so a page cannot ask for something a shell cannot. The terminal asks it off the list one key opens, beside the other questions that take nothing. |
...
| `update` | `update` | dashboard | Reachable in full from all three, and read twice on each of them: unconfirmed it names every service that would move, the version it stands on, the version this build pins, how large the step is and which of the steps migrate state and so cannot be walked back; confirmed it takes them one at a time, behind a backup that is a precondition rather than an offer. Nothing here reaches a registry. The stack ships inside the binary with every image at an explicit version, so what a service *would* move to is known before anything is asked of the network and all that is left to find out is which version each one is standing on now — one question to the engine. `--service` narrows it to one, which the action carries under the same name and the dashboard takes off the panel the box is drawn over rather than having typed. `--wait` is the offer made to a stack with something still coming down: updating stops the download clients, and what is part-way down does not always resume where it left off, so a confirmed run that would interrupt one is refused rather than carried out. The command line and a browser say that word outright; the terminal is offered it **under the account that named the transfers**, because whether there is anything to wait for is the download clients' answer rather than something worth asking somebody in advance — the same shape the restore beside it asks its re-point in, and reached through the same field on the same list. What a run leaves with is the state it came to rather than whether it returned: one that halted part-way left the stack on two versions at once, and a script reading success from it would go on as though everything had moved. |
...
On the other side of the table forty reach the terminal in full, four reach
...
line accepts is reachable from a screen, and all but three of them are reachable
whole.
