...
| `migrate` | `/api/migration`, `migrate-adopt` | dashboard | Reachable in full from all three. The survey is the read — the stacks already standing on this machine, the ports they hold that lemonfiber would want, what it could not take over, what taking it over would cost, and what an unlinkable layout costs — and it starts, stops, moves and writes nothing. Adopting is the act beside it, and the only part of migration that writes: one line saying which Compose project lemonfiber manages. Unconfirmed it says what it would do and writes nothing, so what a browser agrees to is what it was shown. The other three modes are described by the survey and not yet offered as actions; when they are, each arrives here as a name of its own rather than as an argument to this one, because standing a second stack up and stopping somebody's are not the same act under a flag. |
