...
| `migrate` | `/api/migration` | dashboard | Reachable in full from all three, and the one read that is about what lemonfiber did *not* build: the stacks already standing on this machine, the ports they hold that lemonfiber would want, and what it could not take over as it stands. It is a survey and nothing else — it starts, stops, moves and writes nothing — which is why the whole of it reaches every surface rather than a reading half. Deciding what to do about what it found is a different thing, an act on somebody's own setup, and each of those will arrive behind a named action. |
