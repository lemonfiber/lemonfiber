...
A row can also lose an argument without any figure moving, and two rows are the
case in point. `quality` read `dashboard, partial` while the dashboard offered none
of its three writes, and reads the same now that it offers all three, because the
media type a choice applies to is left to a browser. `doctor` read it while the
screen showed storage and VPN facts and no verdict about any of them, and reads it
still now that the diagnosis is one of the questions and the checks that disturb a
running system are offered under its answer — because putting what a diagnosis
finds right is `--fix`, `--undo` and `--accept`, and none of the three has a
terminal form yet. That is the shape of the rest of this column's work — the counts say how many requests are reached and how many
...
them are the six writes that are not about what is running at all: the wiring,
keeping an operator's edits, throwing them away, a capture, a bundle and an archive
put back; the two that keep going once they are started; the key that hands the
terminal to the web surface; the three changes to the quality the stack aims for,
which sit apart from the errands because what an agreement means on them is not what
it means there; and the diagnosis widened to the checks that disturb a running
system, which sits apart from all four lists because there is one of it and because
what it is put under is a report rather than a rehearsal. No request the command
line accepts has a terminal form of `none` any more.
