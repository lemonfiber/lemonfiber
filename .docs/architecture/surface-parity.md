...
| `plugin` | intrinsic | intrinsic | **The second honest exception in this table, and the only one that is an exception on both sides.** Everything under this word is a document about *authoring a plugin*, not about operating a stack: the capabilities a service can claim, the places a plugin may extend lemonfiber, and the schema an editor validates `plugin.toml` against. All three are generated at build time and attached to every release, so the reader who wants them is a plugin author at a shell with a text editor open — and the two surfaces that are not that shell have nothing to add. A browser is the wrong place because the answer is identical on every machine and needs no session, no stack and no token to produce; the one already published at a stable URL on each release is the copy a browser should be reading, and a route serving a second one would be lemonfiber standing between an author and a file. A screen is the wrong place for the neighbouring reason: the dashboard is a live picture of what is running, and a JSON Schema pasted into it is a document nobody can copy out of raw mode. It says `intrinsic` rather than `none` in both columns because neither is work somebody is going to do — `none` would claim a gap, and a gap is a promise. The command line reaches all three in full, with no network, no catalogue and no stack running, and each says which generation it is reporting. **The fourth read under this word is not a published document, and it is an exception for a different reason.** `plugin claims` reads a manifest and its recordings *on a path* and says what this build makes of them — which capabilities it claims, whether the recordings it ships demonstrate them, and what asking for each would come to. Its subject is a working copy somebody has open in an editor, which is a thing that exists at a shell and nowhere else: a browser has no path to name, the same reason `--out` is the one thing `support` cannot offer one; and the dashboard is a live picture of what is running, where a plugin under review is not running and is not meant to be. It is also the one request here with an exit status worth branching on, which is an author's CI reading it rather than a person, and neither of the other two surfaces is a CI. |
...
Of the forty-eight requests, forty-three reach the web in full, three reach it in
part, zero do not reach it at all, and two — `ui` and `plugin` — are honest exceptions.
Three gaps and two exceptions is close to the split `G1-R1` asks for, and the split is
...
no terminal form that anybody is still to build — which is where three stood nine
slices ago, nine before the one ahead of that, and twenty before the one that began
it. All but seven of them are reachable whole, and the seventh is `plugin`, which is
the one request that is an exception on both sides rather than a gap on either.
