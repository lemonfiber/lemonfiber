...
On the other side of the table twelve reach the terminal in full, fourteen reach
the terminal in part, and zero have no terminal form — which is where three stood
two slices ago, nine before the one ahead of that, and twenty before the one that
...
Fourteen rows read that way, and with the web column finished they are the whole of
