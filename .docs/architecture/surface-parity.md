...
| `plugin` | none | none | **It was an exception on both sides and it is a gap on both sides now, which is a change in the word rather than in the code.** Five of the words under it are documents about *authoring* a plugin — the capabilities a service can claim, the places a plugin may extend lemonfiber, the schema an editor validates `plugin.toml` against, what one manifest on a path comes to, and what a registry has said about the images it pins. Those five are exceptions and the argument for them has not weakened: three are generated at build time and attached to every release, so a browser should be reading the copy already published at a stable URL rather than a second one served here, and a screen is the wrong place for a JSON Schema nobody can copy out of raw mode; the other two take a path to a working copy somebody has open in an editor, which is a thing that exists at a shell and nowhere else — the same reason `--out` is the one thing `support` cannot offer a browser — and both are read by an author's CI for an exit status, which neither other surface is. **The four that are new are not about authoring, and that is what moved the row.** `plugin install` settles what installing somebody else's plugin decides and writes it down; `plugin installed` reads that record back; `plugin update` replaces one version with another as one operation; `plugin remove` takes one off again. What is installed on an operator's stack is precisely what `F7` says must be visible wherever anything else about that stack is, and a browser is the surface an operator is most likely to be looking at when something surprises them — so a screen and a tab both have something to add here, and neither has it yet. The install and the update take a path and will stay the shell's for the reason the other two path-taking words are; the reading and the removal — which names a plugin by its id — have no such excuse. The row says `none` rather than `intrinsic` because two of its nine words are work somebody is going to do, and an exception left standing over a gap is the failure `G1-R1` exists to prevent — read as *this will never be built* when what is true is *this is not built yet*. |
...
| `hosting` | `/api/hosting`, `hosting-install`, `hosting-remove` | dashboard, partial | Reachable in full from a browser and asked from a screen, and short on the screen of the two words that change it. This is the short request that decides what becomes of the long ones: it hands `watch`, `household expiring` and the start a restart of this machine should make to the operating system's own service manager, so none of them stops with the window it was typed in. The third is on this list for a reason worth stating: two of them outlive a terminal by running for weeks and the third outlives one by being started again at every login, which is the same promise arrived at from the other direction — and what an operator wants to know is what this machine keeps doing for them, not whether a given answer is a process that never exits. **Installed and running are two facts here and are never printed as one** — the reading says what the manager answered, and a manager that would not answer reads as `installed-unverified` rather than as working, which is the state where somebody believes a guarantee is in force and it is not. Both halves sit on the web as comfortably as any other write: what is installed is one small file in the operator's own account, the answer comes back in the same breath rather than behind a job's name, and nothing about it needs a terminal held open — which is the whole distinction between this and the clock it hosts. What the screen is short of is the pair the guard's install takes: one word off a list of three, and then the forms it guards, typed. The errand list carries a name typed, a value taken off a list, and two pairs of the two, and none of them is that pair — so this is `partial` for the reason `bandwidth` is, and closes the same way, with a `Needs` beside `invitation` and `bundling`. The reading is there in full, and the reading is the half that decides whether anybody is misled: a screen that could install a guard and not say whether the machine was running it would be the failure this request exists to remove. |
| `household` | `/api/requests`, `household-allow`, `household-approve`, `household-decline`, partial | dashboard, partial | Served on the web, and asked off the dashboard's own list of questions. Narrowing to one member is offered there now, as a second entry on the same list: taking it opens the line a trace is typed on, and what is typed fills the member a query string carries. Typed rather than taken off a list of the household, because the list it would be taken off is the answer the narrowing exists to avoid asking for — which is the difference between this and the two that are picked. An empty name is refused in the sentence a browser is refused with, which it also was not before: it matched nobody and came back as a household that has asked for nothing, the one reading this report is written to refuse. **It is a panel as well now.** Pending and failing requests are on the screen without being asked for, which is what `D4-R8` asks: a request waiting on a decision, or failed after one, is waiting on the operator, and an operator who has to think to ask is one who finds out when somebody comes to complain. The panel shows only those two states — a request being fetched or already here needs nobody, and listing it would push the ones that do off a panel this size — and the question behind it still carries every member and everything they asked for. Both are built from the one reading, so the screen and the question cannot report different requests. **What each member is held to is on the same line as what they asked for**, because the two are read together: the age limit reads with the certificates the operator's own media server names on either side of it rather than as a bare number, what happens to their unrated content is said whether or not it is on, and a member limited in what they may watch and not in what they may ask for is named in a finding rather than left to be spotted in a column. That last one costs a second service a read, and a service that will not answer costs the agreement rather than the household — an unread answer is not a disagreement. Under the list, where anybody carries a limit at all, the one sentence about what a limit here is not. **What each member may *ask for* is on that line too, and three actions decide it.** Reading who is here and deciding what they may ask for are one subject and one row: the command line spells them as one word with three things under it, and the web as three actions beside the read they change, because a decision is a write and writes are asked for at the other door. There is no fourth endpoint for the reading half — what a household may ask for is part of who is in the household, so it arrives on the read that was already there rather than on one a browser would have to know to ask for. Each member carries the policy they are under, how much of their period is gone, and the day it next makes room; each waiting request carries how long it has been waiting and about what it would cost, labelled a guess because the request service has no notion of size at all. Naming nobody to a choice is the household's own rather than an omission, which is why this is the one action addressed to a person that does not require one. A decline requires a reason and the core carries it inside the variant that refuses, so one cannot be built without it — and the answer says plainly that it does not travel: the request service's own endpoint reads no body and its record has no field for one, so what reaches whoever asked is that it was declined and the reason is the operator's to pass on. An approval is refused where the disk has no room left, in the disk's own words and from the one reading of the volumes every command that brings content onto the disk shares — never as somebody's limit, because raising a limit would change nothing. **One thing under this word reaches neither surface, and it is declared here rather than left to be noticed.** `household expiring` arranges what becomes of the requests nobody rules on, and then closes them for as long as it is left running — the second long-running command this product has, and the only one that acts while nobody is watching. The arranging half would sit on either surface as comfortably as the three beside it; the running half would not, and that is the whole of the gap. A browser is answered with a job's name and lets a name go when nobody redeems it, which is the right arrangement for a guard measured in minutes and the wrong one for a period measured in days: a clock started in a tab and released with the tab is a clock that stops every afternoon, which is worse than one nobody offered. A screen in raw mode has the same shape and the same answer. **What would close it is not on this roadmap either**, and saying so is the point of writing it down: `B8` brings the *stack* back after a reboot and hosts nothing of lemonfiber's own, so a clock that outlives a terminal needs something nothing has asked for yet. Until it does, the command says plainly on every surface that nothing runs it — the reminder on this very reading names the command and says nothing runs it for you — because the failure worth avoiding is not a missing button but a household believing requests are being closed while nothing is closing them. |
...
| `catalogue` | `/api/catalogue` | dashboard | Reachable in full from all three, and the read that answers the question a list of nineteen names raises and cannot settle. It takes no arguments for the reason the enumeration beside it takes none: what a service is *for* is a property of the stack rather than of the caller, so there is nothing to narrow it by — and an operator who cannot tell which of nineteen names matters is asking about all of them rather than about the one they could already name. Three facts travel per service because each is useless without the others: a description says what Bazarr does and nothing about whether its being down is worth getting up for, an absence cost says what is lost and nothing about how the loss ranks against the eighteen beside it, and a criticality on its own is a word with no scale. What the browser gets is the listing the shell prints and not a second copy of it — a page carrying its own descriptions would be describing whichever stack its author had in mind, which is precisely the stack the person reading is not running. The terminal asks it off the list one key opens, beside the other questions that take nothing. What the stack has *dropped* closes the listing rather than living somewhere else, because somebody looking for a service that is no longer here has no reason to know a document about it exists — and it is silent where nothing has gone, which is where every stack starts. It is answered from a manifest that has been checked rather than merely read, which is what lets a replacement be taken at face value: a record naming a successor this stack neither declares nor recorded is refused here rather than listed and left to point at nothing. |
| `wiring` | none | none | **A gap, and the newest one.** It says what this stack wires to what — which service each link asks for a capability from, which of them are kept to a named service and why, and what is asked for that nothing provides — and its one verb changes which service fills a capability, which is the whole of substituting one for another. Both halves belong on a browser at least as much as here: the listing is a picture of the stack rather than of a path, and choosing a filler is a setting change, which is the one kind of write the web already offers most of. Neither is here because this slice built the model, the manifest that declares the links and the one surface that proves both work; a route and an action are the next thing to write, not an argument anybody has to win. The terminal is the same answer for the same reason — the listing is a question with no arguments, which is the shape every question on that list already has. Nothing about either is an exception: the only reason to read this row twice is to notice that it should stop being true. |
...
| `provenance` | `/api/provenance` | dashboard | Reachable in full from all three, and the read that turns an assurance into something somebody can check. It takes no arguments for the reason the enumeration above it takes none: what is bundled is a property of the stack rather than of the caller, so there is nothing to narrow it by — and half an answer to *is everything here open source* is the wrong half whichever half it is. The three facts travel together because each is close to useless alone: an identifier nobody can attach to a project is a string, a project without the pinned image is a claim about whatever that project's front page says today, and a pin with neither beside it is a version of nothing in particular. What the browser gets is the list the shell prints and not a second copy of it — a page carrying its own table of licences would be a second account of what is bundled, which is precisely the thing somebody reading this is trying not to have to take on trust. The terminal asks it off the list one key opens, beside the other questions that take nothing. It is answered from a manifest that has been checked rather than merely read, which is what lets the answer say the identifiers are enforced instead of recorded: a stack declaring a licence outside the OSI list is refused here rather than listed with the rest. |
...
| `update` | `/api/update`, `update` | dashboard | Reachable in full from all three, and one request with the object beside it rather than two: moving something forward is one word an operator reaches for, and which of the two things can be moved is what tells them apart. **Naming none is refused rather than answered**, which is where this parts company with the removal — that one has a reading which takes nothing, and here neither object is the smaller case of the other. A page that asked about the stack and was handed the binary has been answered a question it did not ask, and the two answers do not even resemble each other. On the command line clap refuses it where it was typed, which is the only place somebody can be told what the two objects are while they are still typing; on the web it is refused with the rest of the requests that could never have been answered as they stand; on the terminal the two are two entries on the list one key opens, because a single entry that made somebody choose again would be asking them the question it had just asked. **`what=stack`** is read twice: unconfirmed it names every service that would move, the version it stands on, the version this build pins, how large the step is and which of the steps migrate state and so cannot be walked back; confirmed it takes them one at a time, behind a backup that is a precondition rather than an offer. Nothing there reaches a registry — the stack ships inside the binary with every image at an explicit version, so what a service would move to is known before anything is asked of the network and all that is left to find out is which version each one stands on now. `--service` narrows it to one, which the action carries under the same name and the dashboard takes off the panel the box is drawn over rather than having typed. `--wait` is the offer made to a stack with something still coming down: updating stops the download clients, and what is part-way down does not always resume where it left off, so a confirmed run that would interrupt one is refused rather than carried out — said outright by the command line and a browser, and offered on the terminal under the account that named the transfers. What such a run leaves with is the state it came to rather than whether it returned, and a run that moved everything and could not start the stack again says so and is not a success. **`what=self`** replaces nothing anywhere, and that is the whole design of it: no surface replaces the binary, because the thing that would go wrong is silent and arrives weeks later — a copy that overwrote itself underneath Homebrew leaves the next `brew upgrade` producing something the operator did not ask for and cannot explain. So all three answer the same question and hand back the same line to copy: which tool owns this copy, whether anything newer has been released, and the exact command for that tool. `--to` is one query parameter and one flag and the only thing that half takes — it asks about one particular version instead of whatever is newest, which is how going back is asked for, and it carries the sentence saying whether that version reads the configuration already here. |
| `alerts` | `/api/alerts`, partial | dashboard | Reachable in part from the web and in full from the other two. The reading reaches all three — the preset in force, what it means, and any event kind set apart from it. Choosing a preset does not: it is an act on what reaches somebody, and one that makes the product quieter, so a fault would go unmentioned until they next looked. That belongs behind a named action rather than a door a browser opens by asking, and the action is not built yet. |
| `migrate` | `/api/migration`, `migrate-adopt`, `migrate-beside`, `migrate-replace`, `migrate-import` | dashboard | Reachable in full from all three. The survey is the read — the stacks already standing on this machine, the ports they hold that lemonfiber would want, what it could not take over, what taking it over would cost, and what an unlinkable layout costs — and it starts, stops, moves and writes nothing. Adopting is the act beside it, and the only part of migration that writes: one line saying which Compose project lemonfiber manages. Unconfirmed it says what it would do and writes nothing, so what a browser agrees to is what it was shown. Standing beside is the second act: it writes a Compose file of lemonfiber's own, layered over the stack's, saying where each service listens instead so the two do not want the same ports — and touches nothing of theirs at all. Replacing is the third: it stops the containers of the existing project and deletes none of them, so the old stack can be started again, and a guard of its own asserts it reaches the runner but never the eraser or the volume. Importing is the fourth: it reads the records the existing services hold — where releases are found, and the three libraries the *arrs follow — and puts the ones lemonfiber does not already hold to lemonfiber's own services, matching by name and resolving every numbered thing again at the far end. Each act arrives here as a name of its own rather than as an argument to one of the others, because standing a second stack up and stopping somebody's are not the same act under a flag. |
| `credentials` | `/api/credentials`, partial | dashboard | Reachable in part from the web and in full from the other two, and the row where the gap is the point rather than an omission. The reading reaches all three: every secret the stack holds, what authenticates with each, where the value lives, where it stands, and — printed every time — what keeping them in files does and does not protect against. It carries no value on any surface, and not by filtering: the shape the inventory is built from has no field a value could go in, so a surface that rendered everything it was given would still render none. The two things that can be asked of a line of it are the terminal's alone. **Printing one** puts a credential somewhere it was not before, and over the web that somewhere is a browser cache, whatever proxy is in between and the log each of them keeps — so there is no route and no action that could ask for it, rather than a route that refuses. A terminal prints it in front of the person who typed the confirmation, which is the whole of who should see it. **Replacing one** is refused there for a plainer reason: it is a write that a request could be forged into making, and the thing it would destroy is the credential the stack is currently working on. Neither is `excepted`, because an exception is short of something nobody is going to build and both of these are things somebody could build once the web surface has a confirmation shape worth trusting with them. On the terminal the reading is off the list one key opens, beside the other questions that take nothing. |
...
| `uninstall` | `/api/uninstall`, `uninstall` | dashboard, partial | Reachable in full from a browser and read from a screen, and short on the screen of the half that removes. Four removals rather than a scale, each explicitly chosen: `stop` takes nothing, `services` takes the containers, their network and the images pulled for them, `configuration` takes each service's own settings and everything lemonfiber keeps with the credentials in both, and `media` takes the library. The web needs both halves named here for the reason `space` does — `/api/uninstall` is the listing, and the action of the same name is the answer to it, reaching one command that reads the same way twice. **What the operator agrees to is the list, not a summary**: every container, image and path, each with what it occupies and, where it is being left, why — because a count is something to agree to and a list is something to check. The one removal that reaches the library takes no bare yes. The listing names itself, `--agreed` answers that name, and the name is built over the size in the words it was shown in, so a yes given against one reading of a disk cannot be spent on another. What the screen is short of is exactly that half. A removal that cannot be undone is not a thing to reach by pressing enter at a list, and the listing — which is the half that decides whether anybody is misled — is there in full, asked with the one word that says which removal. It closes the way `hosting` does, with a `Needs` beside `invitation` and `bundling`, if it is ever decided that it should. |
...
| `held` | `/api/held` | dashboard | Reachable in full from all three, and the read that answers the other half of what somebody opens this product for. `household` says what has been *asked for*; this says what is already here, which nothing else could say — the libraries read names the containers the media server keeps and never their contents. **Whose shelf is the whole of what it takes, and naming nobody is refused rather than read as everybody.** There is no everybody: the server applies that account's age limit, its blocked kinds and the libraries it may reach before it answers, so one shelf for the household would be wrong for whoever it was not read as. That is also why the narrowing is not a filter this product applies — the account is in the path of the request, so what comes back is what they may see because the server said so, and lemonfiber holds no second copy of three rules to disagree with it on the day any of them moves. An operator reads somebody else's shelf by naming them, which is the only honest way to see what a limit set on the household screen actually comes to: a limit is a claim about what somebody may watch, and this is the list. How much of it to answer with is the caller's, bounded, and **refused rather than rounded at either end** — somebody who asked for five thousand and was handed five hundred has been told that is the shelf, and a narrower answer wearing the shape of the answer is the same defect as a wider one. The terminal takes the bound from the served read rather than restating it, so a browser and a shell looking at one household cannot come to show two different shelves. **An empty shelf and an unread one never read alike**: a server that would not answer comes back marked unread with the reason on it, because the alternative tells a household they own nothing on the day the media server rebooted. The terminal asks it off the list one key opens, beside what the household asked for, and taking it opens a line to type the name on — typed rather than picked for the reason a member is typed on the question beside it. |
| `history` | `/api/history` | dashboard | Reachable in full from all three, and it is the read that makes a record already being written worth keeping. Every apply, seed and reconfigure journals what it did and the values on both sides, and until now nothing could look at it: the file was written for `undo` to replay and read by nothing an operator could ask. So it answers with every change newest first, what each did in the operator's own terms rather than the journal's, and how far each could be put back — in full, in part, or not by lemonfiber, with the reason where it is not and what to do instead. The judgement is the core's and not a surface's, so all three say the same thing about the same change. Putting one back is its own request, `undo`, in the row below: the record had to be readable before an operator could choose which entry to act on, which is the order these two arrived in rather than an omission. What the read refuses to promise is the useful half: a setting whose value has since been changed by hand says so and offers the previous value rather than writing over what somebody else put there, and a change a later one depends on says which. The horizon is stated with the entries, because a record that has been trimmed and one that has always been short are the same list otherwise. |
| `undo` | `undo` | dashboard, partial | Reachable in full from all three, and it is the act the read above exists to make choosable. A stamp names one entry in the history and, through it, the whole **run** that entry belongs to — the operation and the stamp together, which is the unit an operator agreed to. Naming a change and getting its run is the behaviour rather than a convenience: undoing half of a seed leaves a machine in a state nobody chose, and the read says beside each entry how many changes would go with it, so what is agreed to is what was shown. Nothing is touched until every change of the run has been judged — a reversal that carried out three of five and then met one it could not is a machine in a state nobody has been told about — and where the judgement refuses, the refusal carries the reason and what to do instead rather than a bare no. What does go back is reported beside what did not: a change only a service can undo, where that service would not answer, is named rather than counted, and so is a setting somebody has chosen since. The reversal is journalled under its own operation, so it appears in the next reading of the history and can be put back in turn. The stamp is typed on every surface rather than picked, for the reason a member's name is: the list it would be picked off is the history itself, which is the read beside it. **Partial on the terminal**: the dashboard's errand list offers the no-stamp form and no other, because the stamp would have to be typed and the list it would be picked off is the history, which that screen does not carry. Sending no stamp at all is what that errand and `doctor --undo` both send, and means the last repair — the question this action answered before a run could be named, kept because the dashboard's errand still asks it. |

## What the table adds up to

Of the fifty requests, forty-four reach the web in full, three reach it in
part, two do not reach it at all, and one — `ui` — is an honest exception.
Five gaps and one exception is close to the split `G1-R1` asks for, and the split is
deliberately lopsided: an exception has to survive being argued, and almost nothing
does.

**One of those figures moved because an exception stopped being true, which is the
direction this page is meant to move in.** `plugin` was an exception on both sides
and was right to be while everything under the word was a document about *authoring*
one. It is not any more: two of the words under it are about this machine — what is
installed on it, and what putting something on it decided — and a browser showing an
operator what a stranger's plugin is doing on their stack is exactly the thing `F7`
asks for. So the row is a gap now rather than an argument, and it says `none` on both
sides because that is what is reachable there today. An exception that has stopped
being arguable and is left standing is worse than a gap, because a gap is at least
counted.

**The web column was finished and is not any more, and what reopened it is worth
reading rather than counting.** It stood at nothing out of a browser's reach and
nothing half in it for several slices, and the row that broke that is `household` —
not because a browser is unsuited to what the new half asks, but because the new
half is the first thing this product does *while nobody is watching*. `household
expiring` closes the requests nobody ruled on, for as long as it is left running.
The arranging half of it would sit on either surface as comfortably as the three
beside it; the running half is a clock measured in days, and both of the other two
surfaces hold a long thing only while somebody is still attending to it — a browser
by redeeming a job's name, a screen by staying in raw mode. Offering it there would
be offering a clock that stops every afternoon, which is worse than not offering it,
and worst of all on the one command whose failure mode is a household believing
requests are being closed while nothing is closing them.

**That row is still `partial`, and what it is short of has changed.** It used to be
short of something nothing on the roadmap supplied — `B8` brings the *stack* back
after a reboot and hosts nothing of lemonfiber's own — and there is a request that
does now: `hosting` hands the clock to this machine's own service manager, and it
is reachable in full from a browser. So the running half is no longer something only
a held-open terminal can have; it is something asked for once, at a request of its
own, which is where an installation that starts at every login belongs. What
`household` is short of is unchanged and unglamorous: the run itself is still not a
thing to start inside a tab or a raw-mode screen, and neither surface pretends
otherwise. What moved is the sentence underneath it. The reminder that names an
...
On the other side of the table forty-two reach the terminal in full, five reach
the terminal in part, one reaches the terminal but for an exception, and two have
no terminal form that anybody is still to build. That last figure stood at zero for
a slice and is at two, which are `wiring` and `plugin` and are gaps rather than
arguments:
three stood there nine slices ago, nine before the one ahead of that, and twenty
before the one that began it. All but eight of them are reachable whole, and the
one that is an exception rather than a gap is `support`, which is whole on a screen
but for the one thing a screen has nowhere to put.

**The web column's second gap is the newer one, and it is a gap by choice.**
`credentials` reaches a browser for the reading and for nothing else, because the
two acts under that word each put a credential somewhere a browser would keep a copy
of it. It is written as `partial` rather than argued into an exception, because what
closes it is a confirmation shape the web surface does not have yet rather than a
principle — and a row that claimed otherwise would be claiming the work is finished.

**Both columns have a gap now, and one of them is the same gap.** The `household`
row is short on both surfaces and for one reason, written out above; a screen that
cannot hold a clock for a month is the same fact as a tab that cannot. The other
two are the terminal's alone, and they are one gap wearing two names: neither
`hosting`'s install nor a bandwidth limit is a shape the errand list has. The newer
of them is `hosting`, short of a word taken off a list of three with the forms typed
after it, and whole on the reading — which is the half that decides whether anybody
is misled. The older one is `bandwidth`: the screen asks
...
**The second exception is the newer one, and it runs in the same direction on both
sides.** `plugin` is where an author asks what they may claim, where they may
contribute, and what shape their manifest takes. Every answer is a document generated
at build time and attached to every release under a stable name, so it is identical on
every machine and needs nothing of this one. That is what makes both other surfaces the
wrong place rather than merely the unbuilt place: a browser asking a running server for
a file already published at a stable URL would be lemonfiber standing between an author
and that file, and a JSON Schema on a raw-mode screen is a document nobody can copy out
of. The argument that it should be `none` on the terminal was made and did not survive
— `none` claims a gap, a gap is a promise somebody will close it, and nobody is going
to put a schema on the dashboard.
...
## The terminal acts on thirteen and asks about eleven
...
stopping, switching, restarting and fetching — and answers the eleven reads it showed
...
stopped on the way, how the line the household shares is being shared, and what
taking lemonfiber off this machine would take. Beside them are the seven writes that are not about what is
