...
On the other side of the table twenty-one reach the terminal in full, five reach
the terminal in part, and zero have no terminal form — which is where three stood
four slices ago, nine before the one ahead of that, and twenty before the one that
...
Five rows read that way, and with the web column finished they are the whole of
